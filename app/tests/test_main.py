...
def test_update_user():
    ...
    response = client.put(f"/users/9999",
        json={"email": "updated@example.com", "name":"ono_updated","is_active":True},
    )
    assert response.status_code == 404, response.text
...
def test_create_order():
    response = client.post(
        "/users/",
        json={"email": "order_user@example.com", "password": "ono","name":"ono"},
    )
    assert response.status_code == 200, response.text
    data = response.json()
    user_id = data["id"]
...
def test_read_order():
    response = client.post(
        "/users/",
        json={"email": "order_read_user@example.com", "password": "ono","name":"ono"},
    )
    assert response.status_code == 200, response.text
    data = response.json()
    user_id = data["id"]
...
def test_update_order():
    response = client.post(
        "/users/",
        json={"email": "for_update_order@example.com", "password": "ono","name":"ono"},
    )
    assert response.status_code == 200, response.text
    data = response.json()
    user_id = data["id"]
    ...
    response = client.put(f"/orders/9999",
        json={
...
def test_delete_order():
    response = client.post(
        "/users/",
        json={"email": "for_delete_order@example.com", "password": "ono","name":"ono"},
    )
    assert response.status_code == 200, response.text
    data = response.json()
    user_id = data["id"]
